...
$pdf->Cell(330,6,'REKAPITULASI PERMOHONAN MASA PERSIAPAN PENSIUN (PMPP)',0,1,'C',1);
$pdf->Ln(3);

$pdf->SetFont('Times','B',10);
$pdf->Cell(6,6,'NO',1,0,'C',0);
$pdf->Cell(48,6,'NOMOR SURAT',1,0,'C',0);
$pdf->Cell(33,6,'TANGGAL',1,0,'C',0);
$pdf->Cell(33,6,'TMT',1,0,'C',0);
$pdf->Cell(33,6,'BUP',1,0,'C',0);
...
//$fil = $_GET['filter'];
if($_GET['filter']=='ALL'){
  $data = mysqli_query($konek,"SELECT*FROM pmpp AS a JOIN pegawai AS b ON a.nip=b.nip JOIN pangkat AS C ON b.pangkat=c.id_pnkt ORDER BY a.no_surat DESC");
}else{
  $tg = explode('-', $_GET['filter']);
  $th = $tg[0];
  $bl = $tg[1];
  $data = mysqli_query($konek,"SELECT*FROM pmpp AS a JOIN pegawai AS b ON a.nip=b.nip JOIN pangkat AS C ON b.pangkat=c.id_pnkt WHERE month(tgl_surat)='$bl' AND year(tgl_surat)='$th' ORDER BY a.no_surat DESC");                
}
$no =1;
while($hasil=mysqli_fetch_array($data)){
  ...
  $pdf->Cell(48,($line * $cellHeight),'882/'.$hasil['no_surat'],1,0,'C',0);
  $pdf->Cell(33,($line * $cellHeight),tgl_indo($hasil['tgl_surat']),1,0,'L',0);
  $pdf->Cell(33,($line * $cellHeight),tgl_indo($hasil['tgl_pengajuan']),1,0,'L',0);
  $pdf->Cell(33,($line * $cellHeight),tgl_indo($hasil['tgl_pensiun']),1,0,'L',0);
