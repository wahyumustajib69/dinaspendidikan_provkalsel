...
include 'koneksi.php';
$no = $_GET['ref'];
$sql = mysqli_query($konek,"SELECT*FROM pen_dn AS a JOIN pegawai AS b ON a.nip=b.nip JOIN pangkat AS c ON b.pangkat=c.id_pnkt WHERE no_sur='$no'");
$no =1;
...
$pdf->Cell(55,5,'882/'.$row['no_sur'],0,0,'L',0);
$pdf->Cell(40,5,'',0,0,'C',0);
$pdf->Cell(25,5,'Banjarbaru,',0,0,'L',0);
$pdf->Cell(1,5,'',0,0,'C',0);
$pdf->Cell(38,5,tgl_indo($row['tgs']),0,1,'L',0);
...
$Height=5; //tinggi sel satu baris normal
  ...
  if($pdf->GetStringWidth($row['alm_pen']) < $Width){
    $line=1;
  }else{
    
    $Length=strlen($row['alm_pen']);  //total panjang teks
    ...
    while($Char < $Length){ //perulangan sampai akhir teks
      ...
      ($Char+$max) < $Length ) {
        $max++;
        $String=substr($row['alm_pen'],$Char,$max);
      }
      //pindahkan ke baris berikutnya
      $Char=$Char+$max;
      //kemudian tambahkan ke dalam array sehingga kita tahu berapa banyak baris yang dibutuhkan
...
$xPos=$pdf->GetX();
$yPos=$pdf->GetY();
$pdf->MultiCell($cellWidth,$cellHeight,$row['alm_pen'],0);
...
$pdf->Cell(60,5,'(MPP) yang bersangkutan mulai tanggal '.tgl_indo($row['thmt']).' dengan masa kerja ' .$ms->y.' Tahun '.$ms->m.' Bulan',0,1,'L',0);

$pdf->Cell(26,5,'',0,0,'L',0);
$pdf->Cell(3,5,'',0,0,'C',0);
$tl = new DateTime($row['tgl_lahir']);
$hl = new DateTime();
$dt = $hl->diff($tl);
$pdf->Cell(60,5,'dan usul pensiun karena telah mencapai Batas Usia Pensiun pada usia '.$dt->y.' Tahun '.$dt->m.' Bulan dengan',0,1,'L',0);

$pdf->Cell(26,5,'',0,0,'L',0);
$pdf->Cell(3,5,'',0,0,'C',0);
$pdf->Cell(60,5,'masa kerja '.$mp->y.' Tahun '.$mp->m.' Bulan.',0,1,'L',0);
