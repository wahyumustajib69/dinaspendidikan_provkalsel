...
$pdf->Cell(330,6,'REKAPITULASI PENSIUN DINI',0,1,'C',1);
$pdf->Ln(3);
...
$pdf->Cell(33,6,'NO. REKOMEND',1,0,'C',0);
$pdf->Cell(33,6,'NO. PNGANTAR',1,0,'C',0);
...
include 'koneksi.php';
if($_GET['filter']=='ALL'){
  $data = mysqli_query($konek,"SELECT*FROM pen_dn AS a JOIN pegawai AS b ON a.nip=b.nip JOIN pangkat AS c ON b.pangkat=c.id_pnkt ORDER BY no_sur DESC");
}else{
  $tg = explode('-', $_GET['filter']);
  $th = $tg[0];
  $bl = $tg[1];
  $data = mysqli_query($konek,"SELECT*FROM pen_dn AS a JOIN pegawai AS b ON a.nip=b.nip JOIN pangkat AS c ON b.pangkat=c.id_pnkt WHERE month(tgs)='$bl' AND year(tgs)='$th' ORDER BY no_sur DESC");
}
$no =1;
while($hasil=mysqli_fetch_array($data)){
  ...
  $pdf->Cell(48,($line * $cellHeight),'882/'.$hasil['no_sur'],1,0,'C',0);
  $pdf->Cell(33,($line * $cellHeight),tgl_indo($hasil['tgs']),1,0,'C',0);
  $pdf->Cell(33,($line * $cellHeight),$hasil['no_recom'],1,0,'C',0);
  $pdf->Cell(33,($line * $cellHeight),$hasil['no_spn'],1,0,'C',0);
